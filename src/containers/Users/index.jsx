...
export default function UserList() {
	...
	const handleDelete = username => {
		...
		dispatch(updateUsers(usersListData?.data?.filter(user => user.username != username)));
	};
	...
	return (
		<div className="userList">
			{(usersListData?.loading || usersListData?.deleteUser?.loading) && <Loader />}
			{(usersListData?.error || usersListData?.deleteUser?.error) && (
				<AlertPopup type="error" message="a problem occured" />
			)}
			{usersListData?.deleteUser?.success && <AlertPopup type="success" message="user deleted" />}
